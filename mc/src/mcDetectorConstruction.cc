...
G4VPhysicalVolume *mcDetectorConstruction::Construct()
{
    ...
    G4Material *G4_POLYSTYRENE = nistManager->FindOrBuildMaterial("G4_POLYSTYRENE");
    ...
    G4Box *solidDetectorArray = new G4Box("DetectorArray", 80 * cm, 80 * cm, 50 * cm);
    G4LogicalVolume *logicDetectorArray = new G4LogicalVolume(solidDetectorArray, // its solid
                                                              G4_AIR,             // its material
                                                              "DetectorArray");   // its name

    new G4PVPlacement(0, G4ThreeVector(0 * cm, 0 * cm, 5 * m), logicDetectorArray, "DetectorArray", logicWorld, false, 0);
    ...
    //////////////////////////
    G4Box *PolystyreneBox = new G4Box("PolystyreneBox", 90 * cm, 90 * cm, 60 * cm);
    G4Box *EmptyFieldBox = new G4Box("EmptyFieldBox", 80 * cm, 80 * cm, 50 * cm);

    G4VSolid *SolidStyrofoam = new G4SubtractionSolid("Styrofoam", PolystyreneBox, EmptyFieldBox, 0, G4ThreeVector(0, 0, 0));
    G4LogicalVolume *logicStyrofoam = new G4LogicalVolume(SolidStyrofoam, // its solid
                                                          G4_POLYSTYRENE, // its material
                                                          "Styrofoam");   // its name

    new G4PVPlacement(0, G4ThreeVector(0 * cm, 0 * cm, 5 * m), logicStyrofoam, "Styrofoam", logicWorld, false, 0);
